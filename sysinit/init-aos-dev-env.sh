...
install_necessary_software_for_develop_environment(){
    # Install the necessary software
    echo "Installing the necessary software for develop environment..."
    apt install -y vim \
        ssh \
        bash-completion \
        sudo \
...
setup_virtual_environment(){
    ...
    # Using uv to install necessary software for ansible controller
...
read -p "Do you want to setup the virtual environment in the current directory? [y/n] " choice
if [ $choice == "y" -o $choice == "Y" ]; then
    setup_virtual_environment
fi
